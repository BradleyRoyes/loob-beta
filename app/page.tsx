...
export default function Page() {
  ...
  // Updated to use input state directly for transcriptions
  ...
  const handleTranscription = (transcription: string) => {
    // Set transcription directly to input field for user to send manually
  ...
  const handleSend = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Optionally clear the input field after send
    handleInputChange({ target: { value: "" } } as React.ChangeEvent<HTMLInputElement>);
  };

  const handlePrompt = (promptText: string) => {
    append({ id: sessionId, content: promptText, role: "user" as const }); // Ensure role is typed correctly
  };
