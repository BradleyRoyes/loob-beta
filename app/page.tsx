...
export default function Page() {
  ...
  const handleTranscription = (transcription: string) => {
    handleInputChange({ target: { value: transcription } } as React.ChangeEvent<HTMLInputElement>);
  };
  ...
  const handlePrompt = (promptText: string) => {
    handleInputChange({ target: { value: promptText } } as React.ChangeEvent<HTMLInputElement>);
  };
...
if (showNeuronVisual) {
  return (
    <>
      <NeuronVisual />
      <button onClick={() => setShowNeuronVisual(false)} className="fixed top-4 right-4 p-2 bg-blue-500 text-white rounded">Back to Chat</button>
    </>
  );
}
      ...
      <div className="chatbot-header pb-6">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-1">
            <ThemeButton />
            <button onClick={() => setConfigureOpen(true)} className="configure-button">
              {/* Configuration Button SVG or Icon */}
            </button>
            <button onClick={() => setShowNeuronVisual(true)} className="mb-4 p-2 bg-green-500 text-white rounded">Data Analytics Dashboard</button>
          </div>
        </div>
